...
class RevelSystemsAPI {
    constructor(url = '', apiKey = '', apiSecret = '') {
        ...
        this.CustomerEstablishment = 'resources/CustomerEstablishment/';
        this.CustomerGroup = 'resources/CustomerGroup/';
        this.CustomerGroupCustomers = 'resources/CustomerGroupCustomers/';
        this.CustomerHistory = 'resources/CustomerHistory/';
        this.DeclaredTips = 'resources/DeclaredTips/';
        this.Department = 'resources/Department/';
        this.Device = 'resources/Device/';
    }

    async getDeviceByID(id) {
        return await get(this.url, `${this.Device}${id}/`, this.headers);
    }

    async downloadDevice(
        printer_pos_or_items = '',
        uuid = '',
        establishment = '',
        device_type = '',
        active = '',
        ...
        let url = this.Device + '?';
        if (printer_pos_or_items) url += `printer_pos_or_items=${printer_pos_or_items}&`;
        if (uuid) url += `uuid=${uuid}&`;
        if (establishment) url += `establishment=${establishment}&`;
        if (device_type) url += `device_type=${device_type}&`;
        if (active) url += `active=${active}&`;
        if (ip_address) url += `ip_address=${ip_address}&`;
        if (additional_filters) url += additional_filters;
        return await download(this.url, url, this.headers, 'objects', 'meta');
    }

    async getDepartmentByID(id) {
        return await get(this.url, `${this.Department}${id}/`, this.headers);
    }

    async getDeclaredTipsByID(id) {
        return await get(this.url, `${this.DeclaredTips}${id}/`, this.headers);
    }

    async downloadDeclaredTips(
        employee = '',
        date_range_from = '',
        establishment = '',
        date_range_to = '',
        ...
        let url = this.DeclaredTips + '?';
        if (employee) url += `employee=${employee}&`;
        if (date_range_from) url += `date_range_from=${date_range_from}&`;
        if (establishment) url += `establishment=${establishment}&`;
        if (date_range_to) url += `date_range_to=${date_range_to}&`;
        if (additional_filters) url += additional_filters;
        return await download(this.url, url, this.headers, 'objects', 'meta');
    }

    async getCustomerHistoryByID(id) {
        return await get(this.url, `${this.CustomerHistory}${id}/`, this.headers);
    }

    async downloadCustomerHistory(
        cc_last_name = '',
        now = '',
        last_4_cc_digits = '',
        cc_first_name = '',
        ...
        let url = this.CustomerHistory + '?';
        if (cc_last_name) url += `cc_last_name=${cc_last_name}&`;
        if (now) url += `now=${now}&`;
        if (last_4_cc_digits) url += `last_4_cc_digits=${last_4_cc_digits}&`;
        if (cc_first_name) url += `cc_first_name=${cc_first_name}&`;
        if (id) url += `id=${id}&`;
        if (additional_filters) url += additional_filters;
        return await download(this.url, url, this.headers, 'objects', 'meta');
    }

    async getCustomerGroupCustomersByID(id) {
        return await get(this.url, `${this.CustomerGroupCustomers}${id}/`, this.headers);
    }

    async downloadCustomerGroupCustomers(customer_group = '', id = '', additional_filters = '') {
        let url = this.CustomerGroupCustomers + '?';
        if (customer_group) url += `customer_group=${customer_group}&`;
        if (id) url += `id=${id}&`;
        if (additional_filters) url += additional_filters;
        return await download(this.url, url, this.headers, 'objects', 'meta');
    }
    ...
    async getCustomerGroupByID(id) {
        return await get(this.url, `${this.CustomerGroup}${id}/`, this.headers);
    }

    async downloadCustomerGroup(additional_filters = '') {
        let url = this.CustomerGroup + '?';
        if (additional_filters) url += additional_filters;
        return await download(this.url, url, this.headers, 'objects', 'meta');
    }

    async getCustomerEstablishmentByID(id) {
        return await get(this.url, `${this.CustomerEstablishment}${id}/`, this.headers);
    }

    async downloadCustomerEstablishment(customer = '', establishment = '', additional_filters = '') {
        let url = this.CustomerEstablishment + '?';
